...
def run_mrc(
    ...
    # 모델의 저장 위치 및 run name에 사용할 해시를 생성합니다.
    run_name = make_run_name(data_args.admin)

    # 기존의 출력을 덮어쓰기 하지 않는다면 새로운 output_dir을 지정합니다.
    ...
    # Evaluation
    if training_args.do_eval:
        training_args.output_dir = os.path.join('./predictions/eval', run_name)
...
if __name__ == "__main__":
    ...
    for train_idx, valid_idx in kf.split(org_train):
